...
void MPU6050_init(Ascale scale, uint8_t threshold,
    uint8_t threshold_duration, uint8_t threashold_neg_dec)
{
    uint8_t c;
...
//  delay(100); // Delay 100 ms for PLL to get established on x-axis gyro; should check for PLL ready interrupt  
    ...
    writeByte(MPU6050_ADDRESS, ACCEL_CONFIG, c | scale << 3);                  // Set full scale range for the accelerometer 
...
// vince
    ...
    writeByte(MPU6050_ADDRESS, MOT_DUR, threshold_duration);                   // in ms (LSB = 1 ms)
    writeByte(MPU6050_ADDRESS, MOT_THR, threshold >> 2);                       // in mg (LSB = 2 mg)
    writeByte(MPU6050_ADDRESS, MOT_DETECT_CTRL, threashold_neg_dec);           // 0 = reset all count on below-threshold sample, 1 = decrement by 1, 2 = dec by 2, 3 = dec by 4
...
void MPU6050_calibrate(float *dest2)
{
...
void MPU6050_read_accelerator_data(int16_t * destination)
{
